...
# 06/26/2025 - Function to get DOWN rides
def get_down_attractions(conn, park_name, timestamp):
    start = (timestamp - timedelta(minutes=5)).isoformat()
    ...
    """
    df = pd.read_sql_query(query, conn, params=(park_name, start, end))
    return df.drop_duplicates(subset=['name']) if not df.empty else pd.DataFrame()
...
def update_snapshot(_):
    ...
    try:
        ...
        for park_name, info in parks.items():
            ...
            # NEW: Get Down attractions
            down_df = get_down_attractions(conn, park_name, latest_time)

            # Attractions over 10 mins table
            ...
            closed_list = html.Ul([
                html.Li(
                    ...
                    style={"fontSize": "0.85rem", "fontStyle": "italic", "color": "#6c757d"}
                )
                for _, row in closed_df.iterrows()
            ]) if not closed_df.empty else html.P("None currently closed.", className="text-muted", style={"fontSize": "0.85rem", "fontStyle": "italic"})

            # NEW: Down Attractions list
            if not down_df.empty:
                down_list = html.Ul([
                    html.Li(name, style={"fontSize": "0.9rem"}) for name in down_df['name'].sort_values()
                ])
            else:
                down_list = html.P("No attractions temporarily down.", className="text-muted", style={"fontSize": "0.85rem", "fontStyle": "italic"})

            # Compose the park snapshot column
            snapshot_rows.append(
                dbc.Col([
                    html.H5(park_name, className="text-center mt-2"),
                    table,
                    html.Div([
                        html.P(f"Longest Wait: {longest[0]} – {longest[1]} min", style={"margin": "2px 0"}),
                        html.P(f"Shortest Wait: {shortest[0]} – {shortest[1]} min", style={"margin": "2px 0"}),
                        html.P(f"Operating Attractions: {percent_operating}%", style={"margin": "2px 0"}),
                    ], className="text-muted", style={"fontSize": "0.9rem"}),
                    
                    # Insert DOWN attractions section here:
                    html.Div([
                        html.H6("Attractions Temporarily Down", className="mt-3"),
                        down_list
                    ]),
